 ...
 * Button is enabled based on whether a sync is needed (tracked via SyncContext).
...
export const DocumentsPageHeader = ({ testId = 'documents-page-header' }: DocumentsPageHeaderProps): JSX.Element => {
  const { syncNeeded } = useSyncContext();
  ...
  return (
    <div data-testid={testId} className="flex items-center justify-between">
      ...
      <SyncButton isEnabled={syncNeeded} testId="documents-page-sync-button" />
    </div>
  );
};
